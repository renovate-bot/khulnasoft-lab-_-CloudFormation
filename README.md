...
[![Build Workflow](https://github.com/khulnasoft-lab/cloudformat/workflows/build/badge.svg)](https://github.com/khulnasoft-lab/sim/actions)
[![Deploy Workflow](https://github.com/khulnasoft-lab/cloudformat/workflows/deploy/badge.svg)](https://github.com/khulnasoft-lab/sim/actions)
[![Sonarcloud Status](https://sonarcloud.io/api/project_badges/measure?project=cloudformat&metric=alert_status)](https://sonarcloud.io/dashboard?id=cloudformat)
...
1. Navigate to the desired provisioning directory - either [provision/gke](provision/gke) or [provision/gke-with-gpu](provision/gke-with-gpu).  The [gke](provision/gke) specification creates a "normal" cluster with a single node pool.  The [gke-with-gpu](provision/gke-with-gpu) specification adds Nvidia T4 GPU capabilities to the Jupyter component, for AI/ML/GPU workloads.  If you do not want to enable the Jupyter component, or want it but for non-AI/ML/GPU workloads, then use the [gke](provision/gke) specification.  The [gke](provision/gke) specification is recommended for most users.  Once you've navigated to the desired infrastructure specification directory, provision with:
    1. Using the default zone (us-central1-a) and cluster name (cloudformat):
        ```
        terraform init
        terraform apply
        ```
...
If using Google Kubernetes Engine, execute the commands below.  If you [ran provisioning with the default GCP zone and cluster name](#provisioning-(optional)), then use `cloudformat` as the cluster name and `us-central1-a` as the cluster zone.  Other cloud vendors should provide a similar cli and commands for configuring `kubectl`, if you are not using Google Kubernetes Engine.
```
gcloud init
gcloud container clusters get-credentials cloudformat --zone us-central1-a
```

Next, create a namespace and configure `kubectl` to use that namespace:
```bash
kubectl create namespace cloudformat
kubectl config set-context --current --namespace cloudformat
```

### Elastic Kubernetes Service (AWS)

```bash
aws eks update-kubeconfig --region us-east-1 --name cloudformat
kubectl create namespace cloudformat
kubectl config set-context --current --namespace cloudformat
```
...
cd build
docker build --tag $REPO/cloudformat-code-server:latest ./code-server
docker build --tag $REPO/cloudformat-initializers:latest ./initializers
docker build --tag $REPO/cloudformat-jupyter:latest ./jupyter
docker build --tag $REPO/cloudformat-pgweb:latest ./pgweb
docker push $REPO/cloudformat-code-server:latest
docker push $REPO/cloudformat-initializers:latest
docker push $REPO/cloudformat-jupyter:latest
docker push $REPO/cloudformat-pgweb:latest
cd ..
```
...
sed -i "s|CODE_IMAGE|us.gcr.io/$GOOGLE_PROJECT/cloudformat/code|g" skaffold.yaml
sed -i "s|INITIALIZERS_IMAGE|us.gcr.io/$GOOGLE_PROJECT/cloudformat/initializers|g" skaffold.yaml
sed -i "s|JUPYTER_IMAGE|us.gcr.io/$GOOGLE_PROJECT/cloudformat/jupyter|g" skaffold.yaml
sed -i "s|PGWEB_IMAGE|us.gcr.io/$GOOGLE_PROJECT/cloudformat/pgweb|g" skaffold.yaml
skaffold dev
```
